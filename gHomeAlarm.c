	 ...
	 char *argv[])
{
	...
	zbPollAlarmPoll(ServerIP, ServerPort, AlarmAddress, SleepTime, ConnectionTimer, rdTimeout, OnTime);
	...
	while(1)
	{
		...
		switch(gizigbeeConnectionState)
		{
			...
			case zigBeePOLL:
				...
				{
					rc = readBuffer(iZigbeeSocket, NULL, cBuffer, sizeof(cBuffer), usec);
					switch (rc)
					{
						case ARM :
